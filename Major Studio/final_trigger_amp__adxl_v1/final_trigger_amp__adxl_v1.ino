...
const int zPin = A4; //second song trigger
int trigger[1] = {TRIG1};
...
const int AMP3 = A5; // AMP trigger which is T3
...
void loop()
{
  ...
  zMotion=analogRead(A4);
  delay(1000);
  z2Motion=analogRead(A4);
...
//  ------------------volume due to ADXL and AMP --------------------

  //trigger sound off with a certain frequency
  if (peakToPeak > 150 || abs(zDelta) < 1) {
